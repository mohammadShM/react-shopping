...
export function CreateComment(props) {
    ...
    return <form onSubmit={submitHandler} className="shadow p-5 ">
        <div className="form-group ">
            <label style={styles.text}>نام : </label>
            <input value={state.comment.author || ''} onChange={changeHandlerAuthor} name="author"
                   style={styles.text} type="text" className="form-control"/>
            <small id="emailHelp" style={styles.text} className="form-text mb-4 text-danger">
                {state.errors.author}
            </small>
        </div>
        <div className="form-group">
            <label style={styles.text}>متن خود را وارد کنید : </label>
            <textarea value={state.comment.text || ''} onChange={changeHandlerText}
                      className="form-control mb-4" style={styles.text} rows="5" name="text"/>
            <small id="emailHelp" style={styles.text} className="form-text mb-4 text-danger">
                {state.errors.text}
            </small>
        </div>
